<?php
require 'db.php';
require_once __DIR__ . '/auth_check.php'; // proteger el login y mandarlo a welcome si no ha verificado su email
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if(isset($_FILES['image']) && $_FILES['image']['name'] !== ''){
        ...
        if(!in_array($file_ext, $allowed_ext)){
            ...
            // Usamos la misma configuración que add_product.php
            $uploadDir = 'images/products/';
            if(!is_dir($uploadDir)) mkdir($uploadDir, 0777, true);

            // Nombre similar a add_product: timestamp + original
            $new_name = $uploadDir . time() . '_' . preg_replace('/[^a-zA-Z0-9_\.-]/', '_', $_FILES['image']['name']);
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .page{padding:24px 20px 64px;transition: margin-left .3s ease;}
    ...
    .header{max-width:980px;margin:0 auto 16px;display:flex;align-items:center;gap:14px;}
    ...
    .card{max-width:980px;margin:0 auto;background:var(--panel);border:1px solid var(--border);border-radius:var(--radius);box-shadow:var(--shadow);overflow:hidden}
    .card-head{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:14px 16px;background:linear-gradient(180deg,#ffffff,#f7fafc);border-bottom:1px solid var(--border)}
    .card-body{padding:18px}
    .form-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px}
    .form-row{display:flex;flex-direction:column;gap:6px}
    .label{font-size:13px;font-weight:700;color:#0b1220}
    .input, .select{border:1px solid var(--border);border-radius:12px;background:#fff;color:var(--text);padding:10px 12px;outline:none;font-size:14px;transition:border .2s ease, box-shadow .2s ease}
    .input:focus, .select:focus{border-color:#bfdbfe;box-shadow:0 0 0 4px rgba(191,219,254,.4)}
    .image-section{margin-top:8px;display:grid;grid-template-columns: 160px 1fr;gap:16px;align-items:start}
    .image-preview{width:160px;height:140px;border:1px solid var(--border);border-radius:12px;background:linear-gradient(180deg,#fbfdff,#f2f6ff);display:grid;place-items:center;overflow:hidden}
    ...
    .alert{padding:12px 16px;border-radius:12px;margin-bottom:12px;font-weight:600;border:1px solid #fecaca;background:#fef2f2;color:#991b1b}
    @media (max-width:900px){ .form-grid{grid-template-columns:1fr} .image-section{grid-template-columns:1fr} }
    @media (max-width:560px){ .image-preview{ width:100%; height:200px; } .btn{ width:100%; text-align:center; } }
  </style>
</head>
<body>
  ...
  <script>
    ...
    btnClear.addEventListener('click', () => {
      ...
      if(existingImg && existingImg.src.startsWith('data:')){
        previewBox.innerHTML = `
          <svg viewBox="0 0 24 24" fill="none" stroke="#94a3b8" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" width="40" height="40" aria-hidden="true">
            <rect x="3" y="3" width="18" height="14" rx="2"></rect>
            <path d="m3 13 4-4 3 3 5-5 4 4"/>
